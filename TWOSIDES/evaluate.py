    ...
    def run_model(seed):
        ...
        for e in range(args.n_epoch):
            ...
            if (e+1) % args.epoch_per_test == 0:
                ...
                out_str = 'epoch:%d\tfeat:%s lr:%.6f lamb:%.8f n_batch:%d n_dim:%d layer:%d ent:%s,\t%.4f %.4f %.4f\t %.4f %.4f %.4f' % (e+1, args.feat, args.lr, args.lamb, args.n_batch, args.n_dim, args.length, args.ent, v_roc, v_pr, v_ap, t_roc, t_pr, t_ap)
                if v_pr > best_acc:
                    best_acc = v_pr
                    best_str = out_str
                    if args.save_model:
